...
content = r'''package ir.restaurant.management.data.repository
...
class Phase6AlertIntegrationTest {
    ...
    fun setUp() = runBlocking {
        ...
        authorizer = SessionAuthorizer(database, clock = { now })
    ...
    fun alertsAreBranchLocationScopedTypedAndSnoozeIsDurable() = runBlocking {
        ...
        val ownerAlerts = LocalAlertRepository(database, authorizer, clock = { now })
        ...
        val scoped = LocalAlertRepository(database, authorizer, clock = { now })
        ...
        assertEquals(AlertDrillDownType.INVENTORY, alertA.drillDownType)

        val until = now + 120_000
...
required = [
    "class Phase6AlertIntegrationTest",
    "alertsAreBranchLocationScopedTypedAndSnoozeIsDurable",
    "InventoryBalanceEntity",
    "UserBranchScopeEntity",
    "AlertDrillDownType.INVENTORY",
...
if missing:
    raise SystemExit(f"Phase6 alert integration test missing invariants: {missing}")
print("PHASE6_HOTFIX_04_ALERT_INTEGRATION_TEST=APPLIED")
