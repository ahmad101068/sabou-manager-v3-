package ir.restaurant.management.data.repository

import android.content.Context
import androidx.test.core.app.ApplicationProvider
import androidx.test.ext.junit.runners.AndroidJUnit4
import ir.restaurant.management.data.db.AppDatabase
import ir.restaurant.management.data.db.BranchEntity
import ir.restaurant.management.data.security.SessionAuthorizer
import ir.restaurant.management.domain.assets.AssetAcquisitionSource
import ir.restaurant.management.domain.assets.AssetDraft
import ir.restaurant.management.domain.assets.AssetMaintenanceDraft
import ir.restaurant.management.domain.operations.UserDraft
import ir.restaurant.management.domain.operations.UserRole
import kotlinx.coroutines.runBlocking
import org.junit.After
import org.junit.Assert.assertEquals
import org.junit.Before
import org.junit.Test
import org.junit.runner.RunWith

@RunWith(AndroidJUnit4::class)
class MultiBranchProfitAndLossIntegrationTest {
    private lateinit var database: AppDatabase
    private lateinit var assets: LocalAssetRepository
    private var now = 1_950_000_000_000L
    private val day = 22_300L

    @Before
    fun setUp() = runBlocking {
        database = AppDatabase.createInMemory(ApplicationProvider.getApplicationContext<Context>())
        val authorizer = SessionAuthorizer(database)
        val security = LocalSecurityRepository(database, clock = { ++now }, authorizer = authorizer)
        val ownerId = security.save(null, UserDraft("multi-pnl-owner", "مالک تست چندشعبه", "123456", UserRole.OWNER, "87654321"))
        security.switchUser(ownerId, "123456")
        database.branchDao().insert(BranchEntity(id = 2L, globalId = "test:pnl:branch:2", code = "B2", name = "شعبه دو", createdAtEpochMillis = now, updatedAtEpochMillis = now))
        assets = LocalAssetRepository(database, clock = { ++now }, authorizer = authorizer)
    }

    @After
    fun tearDown() = database.close()

    @Test
    fun branchTwoPnlExcludesBranchOneAndOrganizationExpenses() = runBlocking {
        postMaintenance("P1", 1L, 100_000_000L)
        postMaintenance("P2", 2L, 12_000_000L)
        postMaintenance("ORG", null, 500_000_000L)

        val branch1 = database.accountingDao().branchProfitLoss(1L, day, day)
        val branch2 = database.accountingDao().branchProfitLoss(2L, day, day)
        assertEquals(100_000_000L, branch1.operatingExpensesExcludingPayrollRial)
        assertEquals(12_000_000L, branch2.operatingExpensesExcludingPayrollRial)
        assertEquals(0L, branch2.payrollRial)
    }

    private suspend fun postMaintenance(code: String, branchId: Long?, amount: Long) {
        val assetId = assets.save(
            null,
            AssetDraft(
                assetCode = "MB-$code",
                name = "دارایی $code",
                category = "تجهیزات",
                quantity = 1,
                purchaseEpochDay = day - 1,
                purchaseCostRial = 600_000_000L,
                salvageValueRial = 0,
                usefulLifeMonths = 60,
                location = "محل $code",
                notes = "multi branch pnl fixture",
                acquisitionSource = AssetAcquisitionSource.BANK,
                branchId = branchId,
            ),
        )
        assets.recordMaintenance(
            AssetMaintenanceDraft(
                assetId = assetId,
                serviceType = "سرویس $code",
                serviceEpochDay = day,
                costRial = amount,
                paymentSource = AssetAcquisitionSource.CASH,
            ),
        )
    }
}
