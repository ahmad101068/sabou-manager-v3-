...
patchfix_decoded(){ local n="$1" ps="$2"; local e="${workspace}/phase4-remediation/phase4-hotfix-${n}.patch.xz.b64" p="${workspace}/.phase4-${n}.patch"; test -s "$e"; base64 --decode "$e"|xz -d >"$p"; verify "$p" "$ps"; git -C "$workspace" apply --check --directory="$target" "$p"; git -C "$workspace" apply --directory="$target" "$p"; echo "PHASE4_HOTFIX_${n}=APPLIED"; }
...
patchfix_decoded 07 c97d84826d8c94f237981e356a37b450573985ab9aeed88589f51f6a75fd111d
for f in \
...
grep -Fq 'fun punchAttendance' "${root}/app/src/main/java/ir/restaurant/management/ui/PersonnelViewModel.kt"
grep -Fq 'attendance_punch_in' "${root}/app/src/main/java/ir/restaurant/management/ui/HrPayrollScreens.kt"
grep -Fq 'attendance_correction_reject_' "${root}/app/src/main/java/ir/restaurant/management/ui/HrPayrollScreens.kt"
